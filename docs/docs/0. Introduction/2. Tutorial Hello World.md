...
6. Then run the frontend template build:
`cd akl/frontend && npm install && npx flamingoBuild`

 You can also start the template dev mode by:
 `npx flamingoDev`

7. Your entrypoint is `akl/akl.go`, this is where the application is started.
...
(Or - if you don't have npm installed: `cd akl/frontend && ./build.sh`
