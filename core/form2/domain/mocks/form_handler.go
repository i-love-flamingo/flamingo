// Code generated by mockery v1.0.0. DO NOT EDIT.

package mocks

import context "context"
import domain "flamingo.me/flamingo/core/form2/domain"
import mock "github.com/stretchr/testify/mock"
import web "flamingo.me/flamingo/framework/web"

// FormHandler is an autogenerated mock type for the FormHandler type
type FormHandler struct {
	mock.Mock
}

// GetForm provides a mock function with given fields: ctx, req
func (_m *FormHandler) GetForm(ctx context.Context, req *web.Request) (*domain.Form, error) {
	ret := _m.Called(ctx, req)

	var r0 *domain.Form
	if rf, ok := ret.Get(0).(func(context.Context, *web.Request) *domain.Form); ok {
		r0 = rf(ctx, req)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*domain.Form)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context, *web.Request) error); ok {
		r1 = rf(ctx, req)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// HandleRequest provides a mock function with given fields: ctx, req
func (_m *FormHandler) HandleRequest(ctx context.Context, req *web.Request) (*domain.Form, error) {
	ret := _m.Called(ctx, req)

	var r0 *domain.Form
	if rf, ok := ret.Get(0).(func(context.Context, *web.Request) *domain.Form); ok {
		r0 = rf(ctx, req)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*domain.Form)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context, *web.Request) error); ok {
		r1 = rf(ctx, req)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}
