...
func (g *Graph) GetByTag(tag string) (res []interface{}) {
	for _, o := range g.Tags[tag] {
	...
	for _, o := range objects {
		...
		if o.Name == "" {
			if !isStructPtr(o.reflectType) {
				if o.reflectType.Kind() == reflect.Func && o.reflectType.NumOut() > 0 {
					if g.factory == nil {
						g.factory = make(map[reflect.Type]*Object)
					}
					if g.Logger != nil {
						g.Logger.Debugf("factory %s for %s", o, o.reflectType.Out(0))
					}
					g.factory[o.reflectType.Out(0)] = o
					return nil
				} else if o.reflectType.String() == "http.HandlerFunc" {
					return nil
				}
				return fmt.Errorf(
...
StructLoop:
	for i := 0; i < o.reflectValue.Elem().NumField(); i++ {
		...
		if fieldType.Kind() == reflect.Interface {
			for ftype, factory := range g.factory {
				if ftype.AssignableTo(o.reflectType.Elem().Field(i).Type) {
					newValue := factory.reflectValue.Call([]reflect.Value{})[0]
					newObject := &Object{
					...
					if g.Logger != nil {
						g.Logger.Debugf(
							"assigned newly factory created %s to field %s in %s",
							newObject,
							o.reflectType.Elem().Field(i).Name,
							o,
						)
		...
		if !isStructPtr(fieldType) {
			return fmt.Errorf(
				"found inject tag on unsupported field %s type %s in type %s",
				o.reflectType.Elem().Field(i).Name,
				fieldType,
				o.reflectType,
			)
...
func (g *Graph) populateUnnamedInterface(o *Object) error {
	...
	for i := 0; i < o.reflectValue.Elem().NumField(); i++ {
		...
		if tag.Private {
			...
			return fmt.Errorf(
				"found private inject tag on interface field %s type %s in type %s and i don't have a factory",
				o.reflectType.Elem().Field(i).Name,
				o.reflectType.Elem().Field(i).Type,
				o.reflectType,
			)
		}
